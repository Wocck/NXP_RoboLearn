...
int main(void) {
    ...
    while (1) {
        int ret = pwm_set_dt(&servo, SERVO_PERIOD_NSEC, SERVO_MIN_PULSE_NSEC);
        if (ret) {
            printk("Error: Failed to set PWM, code=%d\n", ret);
            return -1;
        }
        printk("PWM set to 0°\n");
        k_sleep(K_MSEC(1000));

        ret = pwm_set_dt(&servo, SERVO_PERIOD_NSEC, SERVO_MAX_PULSE_NSEC);
        if (ret) {
            printk("Error: Failed to set PWM, code=%d\n", ret);
            return -1;
        }
        printk("PWM set to 180°\n");
        k_sleep(K_MSEC(1000));
    }
